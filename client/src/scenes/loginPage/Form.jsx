...
const Form = () => {
    ...
    const register = async (values, onSubmitProps) => {
        ...
        const savedUserRes = await fetch('http://localhost:3002/auth/register', {
            method: 'POST',
            body: formData
        })
    ...
    const login = async (values, onSubmitProps) => {
        const loggedInRes = await fetch('http://localhost:3002/auth/login', {
            method: 'POST',
        ...
        {({
            ...
            <form onSubmit={handleSubmit}>
                <Box display='grid' gap='30px' gridTemplateColumns='repeat(2, minmax(0, 1fr))' sx={{ '& > div': { gridColumn: isNonMobile ? undefined : 'span 4' }}}>
                    ...
                    <TextField label='password' onBlur={handleBlur} onChange={handleChange} value={values.password} name='password' error={Boolean(touched.password) && Boolean(errors.password)} helperText={touched.password && errors.password} sx={{ gridColumn: 'span 2' }}/>
                </Box>
